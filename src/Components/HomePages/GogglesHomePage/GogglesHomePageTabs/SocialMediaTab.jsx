...
const SocialMediaTab = ({ values, setFieldValue, productData, categoryData, setSearch }) => {
  ...
  return (
    <>
      <SimpleInputField paramsProps={{ mime_type: mediaConfig.image.join(",") }} nameList={[{ name: `[content][social_media][title]`, placeholder: t("EnterTitle"), title: "Title" }]} />
              ...
              <div className="shipping-accordion-custom">
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    <FileUploadField name={`socialMediaBannerImage${index}`} title="Image" id={`socialMediaBannerImage${index}`} type="file" values={values} setFieldValue={setFieldValue} showImage={values[`socialMediaBannerImage${index}`]} helpertext={getHelperText("375x586px")} />
