  ...
  const removeBanners = (index) => {
    if (values["content"]["social_media"]["banners"].length > 1) {
      ...
      filterValue?.forEach((elem, i) => {
        ...
        elem?.redirect_link?.link_type && setFieldValue(`socialMediaBannerRedirectLinkType${i}`, elem?.redirect_link?.link_type);
        elem?.redirect_link?.link && setFieldValue(`socialMediaBannerRedirectLink${i}`, elem?.redirect_link?.link);
      });
    }
  };
              ...
              <div className="shipping-accordion-custom">
                ...
                {active == index && (
                  <div className="rule-edit-form">
                    ...
                    <CommonRedirect values={values} setFieldValue={setFieldValue} productData={productData} categoryData={categoryData} nameList={{ selectNameKey: `socialMediaBannerRedirectLinkType${index}`, multipleNameKey: `socialMediaBannerRedirectLink${index}` }} setSearch={setSearch} />
